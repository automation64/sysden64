...
function module_gnome_nautilus_setup() {
  bl64_dbg_app_show_function "$@"
  local home="$1"
  local module_type="$SYSDEN64_MODULE_TYPE_SHARED"
  local module_name='gnome_nautilus'
  local module_etc=''
  local extra_locations=''

  module_detect "$module_name" 'nautilus' 'Gnome Nautilus - File Manager GUI' "$extra_locations" || return 0

  module_etc="$(module_config_get_source "$module_type" "$module_name")" &&
    module_gnome_nautilus_setup_config "$home" "$module_etc" "$module_name" "$module_type"
}

function module_gnome_nautilus_setup_config() {
  bl64_dbg_app_show_function "$@"
  local home="$1"
  local module_etc="$2"
  local module_name="$3"
