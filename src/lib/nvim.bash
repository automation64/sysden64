...
function module_nvim_setup() {
  ...
  local module_name='nvim'
  local module_etc=''

  module_detect "$module_name" 'nvim' 'NeoVIM - Text Editor' || return 0

  module_etc="$(module_config_get_source "$module_type" "$module_name")" &&
    module_setup_env "$home" "$module_etc" "$module_type" "$module_name" &&
    module_nvim_setup_config "$home" "$module_etc" "$module_name" "$module_type"
}
...
function module_nvim_setup_config() {
  ...
  local module_config='.config/nvim'
