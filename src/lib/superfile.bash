...
function module_superfile_setup() {
  bl64_dbg_app_show_function "$@"
  local home="$1"
  local module_type="$SYSDEN64_MODULE_TYPE_SHARED"
  local module_name='superfile'
  local module_etc=''
  local extra_locations=''

  module_detect "$module_name" 'spf' 'SPF - File Manager' "$extra_locations" || return 0

  module_etc="$(module_config_get_source "$module_type" "$module_name")" &&
    module_superfile_setup_config "$home" "$module_etc" "$module_name" "$module_type"
}

function module_superfile_setup_config() {
  bl64_dbg_app_show_function "$@"
  local home="$1"
  local module_etc="$2"
  local module_name="$3"
  ...
  local module_config='.config/superfile'
