...
function module_keepassxc_setup() {
  ...
  local module_name='keepassxc'
  local module_etc=''

  module_detect "$module_name" 'keepassxc' 'KeePassXC - Password Manager' || return 0

  module_etc="$(module_config_get_source "$module_type" "$module_name")" &&
    module_keepassxc_setup_config "$home" "$module_etc" "$module_name" "$module_type"
}
...
function module_keepassxc_setup_config() {
  ...
  local module_config='.config/keepassxc'
