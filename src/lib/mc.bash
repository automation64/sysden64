...
function module_mc_setup() {
  ...
  local module_name='mc'
  local module_etc=''

  module_detect "$module_name" 'mc' 'MC - File Manager' || return 0

  module_etc="$(module_config_get_source "$module_type" "$module_name")" &&
    module_mc_setup_config "$home" "$module_etc" "$module_name" "$module_type"
}
