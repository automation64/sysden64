...
function module_starship_setup() {
  ...
  local module_name='starship'
  local module_etc=''

  module_detect "$module_name" 'starship' 'Starship - Shell Prompt Manager' || return 0

  module_etc="$(module_config_get_source "$module_type" "$module_name")" &&
    module_setup_env "$home" "$module_etc" "$module_type" "$module_name" &&
    module_starship_setup_config "$home" "$module_etc" "$module_name" "$module_type"
}
...
function module_starship_setup_config() {
  ...
  local module_config='.config/starship.toml'
