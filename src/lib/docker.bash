...
function module_docker_setup() {
  ...
  local module_name='docker'
  local module_etc=''

  module_detect "$module_name" 'docker' 'Docker - CLI' || return 0

  module_etc="$(module_config_get_source "$module_type" "$module_name")" &&
    module_setup_env "$home" "$module_etc" "$module_type" "$module_name" &&
    module_docker_setup_config "$home" "$module_etc" "$module_name" "$module_type"
}

function module_docker_setup_config() {
  bl64_dbg_app_show_function "$@"
  local home="$1"
  local module_etc="$2"
  local module_name="$3"
