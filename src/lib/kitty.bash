...
function module_kitty_setup() {
  ...
  local module_name='kitty'
  local module_etc=''

  module_detect "$module_name" 'kitty' 'KiTTY - Terminal Emulator' || return 0

  module_etc="$(module_config_get_source "$module_type" "$module_name")" &&
    module_kitty_setup_config "$home" "$module_etc" "$module_name" "$module_type"
}
...
function module_kitty_setup_config() {
  ...
  local module_config='.config/kitty'
