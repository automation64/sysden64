...
function module_antigravity_setup() {
  ...
  local model='antigravity'
  local extra_locations=''

  [[ "$BL64_OS_TYPE" != "$BL64_OS_TYPE_LINUX" ]] && return 0
  module_detect "$model" 'antigravity' 'Antigravity - AI IDE' "$extra_locations" || return 0

  source="$(module_set_model "$module_type" "$model")" &&
    module_antigravity_setup_config "$home" "$source" "$model" "$module_type"
}
